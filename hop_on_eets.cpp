// "tick" = Engine_GetSimTick (Simulator::DeterministicFrameAdvance step counter minus per-round baseline);
// falls back to counting mod Update calls if the counter addr is unavailable. The real counter catches
// sub-stepping a per-Update ++ would miss.
#ifdef _WIN32
#include <winsock2.h>
#include <ws2tcpip.h>
#endif
...
extern "C" void EetsMod_OnKey(int key, int mods, int down) { mod_on_key(key, mods, down); }
