...
export function modLineToMsg(line: string, fallbackName = 'anon'): any | null {
  ...
  const a = t.split(/\s+/);
  switch (a[0]) {
    case 'hello':    return { type: 'hello', uuid: a[1] || fallbackName, player_id: a[2] || a[1] || fallbackName };
    case 'host':     return { type: 'host' };
    case 'join':     return { type: 'join', code: a[1] || '' };
    case 'queue':    return { type: 'queue' };
    case 'ready':    return { type: 'ready' };
    case 'build':    return { type: 'build', name: a[1], x: +a[2], y: +a[3] };
    case 'buildend': return { type: 'buildend' };
