...
export function encodeFrame(str: string, mask: boolean): Buffer {
  ...
  header[1] |= 0x80;
  const mk = crypto.randomBytes(4), out = Buffer.alloc(len);
...
export function createParser(onMsg: (s: string) => void, onClose?: () => void): Parser {
  ...
  return (chunk: Buffer) => {
    ...
    for (;;) {
      ...
      if (masked && mk) { const u = Buffer.alloc(len); for (let i = 0; i < len; i++) u[i] = payload[i] ^ mk[i & 3]; payload = u; }
...
export function connect(
  ...
  let upgraded = false, pending = Buffer.alloc(0);
  ...
  socket.on('error', (e: Error) => { if (!upgraded) cb(null, e); });
  return { get conn() { return conn; }, socket };
}
