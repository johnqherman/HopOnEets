...
export function startBridge(opts: BridgeOpts): {
  ...
  const toMod = (line: string) => {
    if (mod && !mod.destroyed) mod.write(line + "\n");
  };
  ...
  function handleModLine(line: string): void {
    ...
    switch (a[0]) {
      ...
      case "ready":
        ...
        break;
      case "build":
        relay.send({ type: "build", name: a[1], x: +a[2], y: +a[3] });
        break;
      case "buildend":
        relay.send({ type: "buildend" });
        break;
      case "pos":
        relay.send({
          type: "pos",
      ...
      case "hash":
        ...
        break;
      case "desync":
        relay.send({ type: "desync", tick: +a[1] });
        break;
      case "finish":
        relay.send({
          type: "finish",
  ...
  function openRelay(): void {
    ws.connect(relayHost, relayPort, (conn, err) => {
      if (err || !conn) {
        log("relay connect failed: " + (err && err.message));
        return;
      }
      relay = conn;
      ...
      const q = pending;
      pending = [];
      q.forEach(handleModLine);
      conn.onJSON((m: any) => {
        switch (m.type) {
          case "match_config":
            toMod(
              `match ${m.match_id} ${m.opponent} ${m.ranked ? 1 : 0} ${m.level ?? -1} ${m.seed ?? 0} ${m.self_r ?? 0} ${m.opp_r ?? 0} ${m.self_rank ?? 0} ${m.opp_rank ?? 0}`,
            );
            break;
          case "countdown":
            toMod(`countdown ${m.seconds} ${m.cap ?? 0}`);
            break;
          case "round":
            toMod(`round ${m.round} ${m.level ?? -1} ${m.seed ?? 0}`);
            break;
          case "room":
            toMod(`code ${m.code}`);
            break;
          case "join_failed":
            toMod(`joinfail ${m.code}`);
            break;
          case "opp_pos":
            toMod(
          ...
          case "opp_finish":
            toMod(
              `oppfin ${m.finish_tick} ${m.completed ? 1 : 0} ${m.items_used}`,
            );
            break;
          case "opp_hash":
            toMod(`oh ${m.tick} ${m.hash} ${m.platform}`);
            break;
          case "no_contest":
            toMod(`nocontest ${m.reason} ${m.tick ?? -1}`);
            break;
          case "result":
          ...
          case "series_over":
            toMod(
              `series ${m.winner} ${m.you_wins} ${m.opp_wins} ${m.ranked ? 1 : 0} ${m.r_old ?? 0} ${m.r_new ?? 0} ${m.forfeit ? 1 : 0} ${m.rank ?? 0}`,
            );
            break;
          case "opponent_left":
            toMod("oppleft");
  ...
  const tcp = net.createServer((sock) => {
    ...
    sock.on("data", (d) => {
      ...
      while ((i = modBuf.indexOf("\n")) >= 0) {
        handleModLine(modBuf.slice(0, i));
        modBuf = modBuf.slice(i + 1);
      }
    });
    sock.on("close", () => {
    ...
    }); // mod quit -> leave match
    sock.on("error", () => {});
  });
  tcp.listen(modPort, "127.0.0.1", () =>
    log(
      `bridge tcp on 127.0.0.1:${modPort} -> relay ${relayHost}:${relayPort}`,
    ),
  );
  openRelay();
  return {
