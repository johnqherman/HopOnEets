...
ws.connect("127.0.0.1", RELAY, (conn, err) => {
  if (err || !conn) {
    console.error("[opp] relay connect failed:", err && err.message);
    process.exit(1);
  }
  let tick = 0;
  ...
  console.log(
    "[opp] queued as GHOSTBOT — in game: F6 -> Online ON -> Ranked queue",
  );

  const startStreaming = () => {
    if (timer) return;
    console.log(
      "[opp] streaming pos + anim state (walks back/forth, cycles emotion/motion/facing)",
    );
    timer = setInterval(() => {
      tick++;
  ...
  conn.onJSON((m: any) => {
    if (m.type === "match_config") {
      console.log("[opp] matched vs " + m.opponent + " — readying");
      conn.send({ type: "ready" });
      startStreaming();
    } else if (m.type === "round") {
      console.log(
        "[opp] round " + m.round + " (level " + m.level + ") — readying",
      );
      conn.send({ type: "ready" });
      ...
      console.log(
        "[opp] countdown " +
          build +
          "s (your build phase) — start a sim to see the ghost",
      );
