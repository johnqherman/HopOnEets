...
#ifdef _WIN32
// Windows: WinHTTP WebSocket; recv blocks -> Win32 thread feeds
// CRITICAL_SECTION inbox that wsc_poll drains. Win32 threads (not std::thread)
// dodge the mingw threading-model dependency; unverified at runtime
#include <windows.h>
#include <winhttp.h>
#include <vector>
...
static void wsc_close() {
  g_whRun = false;
  if (g_whWs)
    WinHttpWebSocketClose(g_whWs, WINHTTP_WEB_SOCKET_SUCCESS_CLOSE_STATUS,
                          nullptr, 0); // unblocks recv thread
  if (g_whThread) {
    WaitForSingleObject(g_whThread, 3000);
    CloseHandle(g_whThread);
    g_whThread = nullptr;
  }
  if (g_whWs) {
    WinHttpCloseHandle(g_whWs);
    g_whWs = nullptr;
  }
  if (g_whConn) {
    WinHttpCloseHandle(g_whConn);
    g_whConn = nullptr;
  }
  if (g_whSession) {
    WinHttpCloseHandle(g_whSession);
    g_whSession = nullptr;
  }
  g_whOpen = false;
  if (g_whCsInit) {
    EnterCriticalSection(&g_whCs);
    g_whInbox.clear();
    LeaveCriticalSection(&g_whCs);
  }
}
...
static DWORD WINAPI wh_recv_loop(LPVOID) {
  ...
  while (g_whRun) {
    ...
    if (WinHttpWebSocketReceive(g_whWs, buf, sizeof(buf), &got, &type) !=
        NO_ERROR)
      break;
...
static bool wsc_connect(const std::string &url) {
  wsc_close();
  WscUrl u = wsc_parse(url);
  if (!u.ok)
    return false;
  if (!g_whCsInit) {
    InitializeCriticalSection(&g_whCs);
    g_whCsInit = true;
  }
  std::wstring hostW(u.host.begin(), u.host.end()),
      pathW(u.path.begin(), u.path.end());
  g_whSession = WinHttpOpen(L"HopOnEets", WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,
                            WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
  if (!g_whSession)
    return false;
  g_whConn =
      WinHttpConnect(g_whSession, hostW.c_str(), (INTERNET_PORT)u.port, 0);
  if (!g_whConn) {
    wsc_close();
    return false;
  }
  HINTERNET req = WinHttpOpenRequest(
      g_whConn, L"GET", pathW.c_str(), nullptr, WINHTTP_NO_REFERER,
  ...
  bool ok =
      ...
      WinHttpSendRequest(req, WINHTTP_NO_ADDITIONAL_HEADERS, 0, nullptr, 0, 0,
                         0) &&
      WinHttpReceiveResponse(req, nullptr);
  if (ok)
    g_whWs = WinHttpWebSocketCompleteUpgrade(req, 0);
...
static void wsc_send_text(const std::string &s) {
  if (g_whOpen)
    WinHttpWebSocketSend(g_whWs, WINHTTP_WEB_SOCKET_UTF8_MESSAGE_BUFFER_TYPE,
...
static WscSSL &wsc_ssl() {
  ...
  for (const char *n : names) {
    s.lib = dlopen(n, RTLD_NOW | RTLD_GLOBAL);
    if (s.lib)
      break;
  }
  if (!s.lib)
  ...
  s.ok = s.client_method && s.CTX_new && s.SSL_new && s.set_fd && s.connect &&
         s.read && s.write && s.get_error;
  return s;
}
...
static int wsc_raw_write(const void *p, int n) {
  return g_wsTls ? wsc_ssl().write(g_wsSsl, p, n)
                 : (int)::send(g_wsFd, p, n, MSG_NOSIGNAL);
}
static int wsc_raw_read(void *p, int n) {
  return g_wsTls ? wsc_ssl().read(g_wsSsl, p, n) : (int)::recv(g_wsFd, p, n, 0);
}
// write all bytes, retrying would-block (TLS WANT_READ/WRITE, EAGAIN) instead
...
static bool wsc_write_all(const char *p, size_t n) {
  ...
  while (o < n) {
    ...
    if (++spins > 2000)
      return false; // ~0.4s would-block = dead
    usleep(200);
  }
  return true;
}
...
static void wsc_close() {
  if (g_wsSsl) {
    wsc_ssl().SSL_free(g_wsSsl);
    g_wsSsl = nullptr;
  }
  if (g_wsCtx) {
    wsc_ssl().CTX_free(g_wsCtx);
    g_wsCtx = nullptr;
  }
  if (g_wsFd >= 0) {
    ::close(g_wsFd);
...
static void wsc_send_frame(unsigned char opcode, const std::string &s) {
  ...
  for (size_t i = 0; i < n; i++)
    f.push_back((char)((unsigned char)s[i] ^ key[i & 3]));
  if (!wsc_write_all(f.data(), f.size()))
    wsc_close();
}
static void wsc_send_text(const std::string &s) { wsc_send_frame(0x1, s); }

// connect: DNS + TCP + (wss) TLS + WS handshake; blocking, then fd set
// non-blocking for wsc_poll
static bool wsc_connect(const std::string &url) {
  wsc_close();
  WscUrl u = wsc_parse(url);
  if (!u.ok)
    return false;
  g_wsTls = u.tls;
  if (g_wsTls && !wsc_ssl().ok)
    return false; // no OpenSSL -> can't do wss
  ...
  if (getaddrinfo(u.host.c_str(), ports, &hints, &res) != 0 || !res)
    return false;
  int fd = -1;
  ...
  if (g_wsTls) {
    WscSSL &o = wsc_ssl();
    g_wsCtx = o.CTX_new(o.client_method());
    if (!g_wsCtx) {
      wsc_close();
      return false;
    }
    g_wsSsl = o.SSL_new(g_wsCtx);
    if (!g_wsSsl) {
      wsc_close();
      return false;
    }
    o.set_fd(g_wsSsl, fd);
    o.ctrl(g_wsSsl, 55 /*SSL_CTRL_SET_TLSEXT_HOSTNAME*/,
           0 /*TLSEXT_NAMETYPE_host_name*/, (void *)u.host.c_str());
    if (o.connect(g_wsSsl) != 1) {
      wsc_close();
      return false;
    }
  }
  ...
  if (!wsc_write_all(req, (size_t)rn)) {
    wsc_close();
    return false;
  }
  ...
  for (int tries = 0; tries < 64 && resp.find("\r\n\r\n") == std::string::npos;
       tries++) {
    int n = wsc_raw_read(buf, sizeof(buf));
    if (n <= 0)
      break;
  ...
  if (hdr != std::string::npos && hdr + 4 < resp.size())
    g_wsRx.assign(resp, hdr + 4, std::string::npos);
  return true;
}
...
static void wsc_poll(void (*onmsg)(const std::string &)) {
  ...
  char buf[4096];
  int n;
  while ((n = wsc_raw_read(buf, sizeof(buf))) > 0)
    g_wsRx.append(buf, (size_t)n);
  if (n == 0 && !g_wsTls) {
    wsc_close();
    return;
  } // plain TCP peer closed
  // parse + consume g_wsRx fully, THEN dispatch: onmsg may close (clearing
  // g_wsRx), which underflowed mid-parse
  std::vector<std::string> msgs, pongs;
  bool gotClose = false;
  size_t off = 0, sz = g_wsRx.size();
  while (off + 2 <= sz) {
    unsigned char b0 = (unsigned char)g_wsRx[off],
                  b1 = (unsigned char)g_wsRx[off + 1];
    int op = b0 & 0x0f;
    ...
    if (len == 126) {
      if (off + 4 > sz)
        break;
      len = ((unsigned char)g_wsRx[off + 2] << 8) |
            (unsigned char)g_wsRx[off + 3];
      hl = 4;
    } else if (len == 127) {
      if (off + 10 > sz)
        break;
      len = 0;
      for (int i = 0; i < 8; i++)
        len = (len << 8) | (unsigned char)g_wsRx[off + 2 + i];
      hl = 10;
    }
    size_t mk = masked ? 4 : 0;
    if (off + hl + mk + len > sz)
      break; // incomplete - wait for more
    std::string payload(g_wsRx.data() + off + hl + mk,
                        (size_t)len); // server frames unmasked
    off += hl + mk + (size_t)len;
    if (op == 0x1 || op == 0x0)
      msgs.push_back(std::move(payload));
    else if (op == 0x9)
      pongs.push_back(std::move(payload)); // ping
    else if (op == 0x8) {
      gotClose = true;
      break;
    } // close
  ...
  if (off)
    g_wsRx.erase(0, off); // consume before any handler runs
  for (auto &p : pongs) {
    if (!g_wsOpen)
      break;
    wsc_send_frame(0xA, p);
  }
  if (gotClose) {
    wsc_close();
    return;
