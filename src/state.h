...
static constexpr int MAX_PLAYER_NAME = 12;   // vanilla "ENTER YOUR NAME" field limit

// ---- match / phase ----
enum Phase { IDLE, BUILD, SIM };
static Phase g_phase       = IDLE;
static bool  g_interRound   = false;   // suppress overlay draws: vanilla victory/screen-transition makes engine draw calls unsafe
static bool  g_matchActive = false;
static bool  g_menuOpen    = false;
static long  g_tick        = 0;
static long  g_engineTickBase = -1;    // Engine_GetSimTick() at sim start; g_tick = now-this (-1 = unavailable, g_tick++ fallback)
static long  g_lastHashBucket = -1;    // sample on bucket change so a tick jump (engine sub-step) can't skip a sample + misalign the compare
static int   g_resets      = 0;
static long  g_finishTick  = -1;
static int   g_roundCounter = 0;
static int   g_youWins = 0, g_ghostWins = 0;   // g_ghostWins = opponent's score ("ghost" = live opponent)
...
static std::string g_playerId = "p1";        // display name (editable); not the identity
static std::string g_playerUuid;             // stable client-generated Elo identity; persisted, never changes
static bool g_matched = false, g_ranked = false;
static std::string g_oppId = "?";
static int g_levelIndex = -1;                // ranked level index from relay (resolved % pool size)
static char g_netMsg[96] = "offline";
static bool  g_liveValid = false; static float g_liveX = 0, g_liveY = 0;
static double g_liveLastTime = 0.0;   // Time() of last opponent pos frame; ghost draws only while fresh
...
static bool g_codeEntry = false; static std::string g_codeBuf;
static bool g_nameEntry = false; static std::string g_nameBuf;
static bool g_confirmForfeit = false;
// reconnect window: mid-match drop holds the relay match briefly so we can rejoin
static bool   g_oppDropped     = false;
static double g_oppDropUntil    = 0.0;    // when opponent's reconnect window ends
static double g_reconnectUntil  = 0.0;    // we dropped: retry until this time, then give up to menu
static double g_lastReconnectTry = 0.0;
static bool g_nameManual = false;   // custom name overrides profile; persisted as save key player_id
...
static int    g_pendingShowdown  = 0;   // consumed at synced countdown so both clients show together
...
static long g_simMaxTicks = 60 * 120;          // DNF watchdog: 120s @60Hz (endless sim -> failed round)
struct GBuild { std::string bp; float x, y; };
...
static long   g_deathTicks  = 0;       // sim ticks in died attempts; added to finish_tick (time penalty)
static bool   g_deathReset  = false;   // handled next Update, off engine's death call stack
static bool   g_selfStop    = false;   // guard: mod-initiated StopSimulation, so its level_reset isn't mistaken for a player Stop
...
static int    g_roundCapSeconds = 180;         // round wall-clock cap -> DNF; relay re-syncs via countdown
static constexpr int g_hubIntroSkip = 1;       // ranked pool: intro levels skipped per hub
...
// garbage guard: reject NaN/inf + absurd coords (stale/half-built Eets reads junk)
